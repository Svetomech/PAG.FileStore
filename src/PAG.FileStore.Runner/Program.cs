...
namespace PAG.FileStore.Runner
{
    public static class Program
    {
        ...
        public static void Main(string[] args)
        {
            ...
            var store2 = new FileStore<ExampleProduct>(); // uses same file on disk

            Console.Write($"Loading store with {N} files from disk... ");
            Console.WriteLine("{0}ms", Benchmark(store2.Load));

            Console.Write($"Removing last file from store with {N} files... ");
            store2.FileRemoved += (s, e) => store2.Save();
            Console.WriteLine("{0}ms", Benchmark(() => store2.Remove(foundProduct)));
        }
